...
shaka.hls.ManifestTextParser = class {
  constructor() {
    ...
    /** @private {!Set<string>} */
    this.mediaPlaylistTagsSet_ = new Set(
        shaka.hls.ManifestTextParser.MEDIA_PLAYLIST_TAGS);
    
    /** @private {!Set<string>} */
    this.segmentTagsSet_ = new Set(
    ...
    /** @private {!RegExp} */
    this.linesSplitRegex_ = /\n+/m;
    
    /** @private {!RegExp} */
    this.headerRegex_ = /^#EXTM3U($|[ \t\n])/m;
    
    /** @private {!RegExp} */
    this.extTagRegex_ = /^(#EXT)/;
  }
  ...
  parsePlaylist(data) {
    ...
    str = str.replace(this.newlineRegex_, '\n').trim();

    const lines = str.split(this.linesSplitRegex_);

    if (!this.headerRegex_.test(lines[0])) {
    ...
    // Combined loop to determine playlist type and parse tags in single pass
    // Whether to skip the next element; initialize to true to skip first elem.
    let skip = true;
    let playlistTypeFound = false;
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      // First pass: determine playlist type if not already found
      if (!playlistTypeFound) {
        if (this.mediaPlaylistTagsSet_.has(tag.name)) {
          playlistType = shaka.hls.PlaylistType.MEDIA;
          playlistTypeFound = true;
        } else if (tag.name == 'EXT-X-STREAM-INF') {
          // This indicates master playlist
          playlistTypeFound = true;
        }
        
        // If this is first loop and we haven't found the type yet,
        // don't increment global id
        if (!playlistTypeFound) {
          this.globalId_ -= 1;
        }
      }
      
      // Check for segment tags (which indicates start of segments section)
      if (this.segmentTagsSet_.has(tag.name)) {
  ...
  parseSegments_(lines, playlistTags) {
    ...
    for (const line of lines) {
      if (this.extTagRegex_.test(line)) {
        const tag = this.parseTag_(line);
        if (this.mediaPlaylistTagsSet_.has(tag.name)) {
  ...
  static parseTag(id, word) {
    ...
    */
    const blocks = word.match(shaka.hls.ManifestTextParser.TAG_REGEX_);
    if (!blocks) {
      throw new shaka.util.Error(
          shaka.util.Error.Severity.CRITICAL,
          shaka.util.Error.Category.MANIFEST,
    ...
    if (data) {
      ...
      const blockValue = parser.readRegex(shaka.hls.ManifestTextParser.VALUE_REGEX_);

      if (blockValue) {
        value = blockValue[1];
      }

      while ((blockAttrs = parser.readRegex(shaka.hls.ManifestTextParser.ATTRIBUTE_REGEX_))) {
 ...
 * @const {!RegExp}
 */
shaka.hls.ManifestTextParser.TAG_REGEX_ = /^#(EXT[^:]*)(?::(.*))?$/;

/**
 * @const {!RegExp}
 */
shaka.hls.ManifestTextParser.VALUE_REGEX_ = /^([^,=]+)(?:,|$)/g;
 ...
 */
 ...
 //   a. A comma
 //   b. End of line
shaka.hls.ManifestTextParser.ATTRIBUTE_REGEX_ = /([^=]+)=(?:"([^"]*)"|([^",]*))(?:,|$)/g;
