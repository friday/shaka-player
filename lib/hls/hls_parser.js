...
shaka.hls.HlsParser = class {
  ...
  createSegments_(playlist, mediaSequenceToStartTime, variables,
    ...
    for (let i = 0; i < hlsSegments.length; i++) {
      ...
      // Pre-compute tag lookups for performance
      ...
      const mapTag = tagLookup.get('EXT-X-MAP');
      ...
      const discontinuityTag = tagLookup.get('EXT-X-DISCONTINUITY');
      if (discontinuityTag) {
      ...
      if (reference) {
        const bitrateTag = tagLookup.get('EXT-X-BITRATE');
        const bitrate = bitrateTag ? Number(bitrateTag.value) : null;
        if (bitrate) {
          bitrates.push({
            bitrate,
